(function(a){
    ...
    a.fn.doAutosize=function(b){
        ...
        })).insertBefore("#"+f+"_addTag");g.push(d);a("#"+f+"_tag").val("");
        if(e.focus){
            a("#"+f+"_tag").focus()
        }else
        {
...
a(f.fake_input).bind("keypress",f,function(b){
    ...
    {        
        ...
        // console.log('cuole')
        // console.log(a(b.data.fake_input).val())
...
f.removeWithBackspace&&a(f.fake_input).bind("keydown",function(b){
    ...
    {        
        ...
        c=c.replace(/[\s]+x$/,"");a("#"+d).removeTag(escape(c));
...
a(f.fake_input).blur();if(f.unique){a(f.fake_input).keydown(function(b){if(b.keyCode==8||String.fromCharCode(b.which).match(/\w+|[áéíóúÁÉÍÓÚñÑ,/]+/)){a(this).removeClass("not_valid")}})}}});return this};a.fn.tagsInput.updateTagsField=function(c,d){var e=a(c).attr("id");a(c).val(d.join(b[e]))};a.fn.tagsInput.importTags=function(d,e){a(d).val("");var f=a(d).attr("id");var g=e.split(b[f]);for(i=0;i<g.length;i++){a(d).addTag(g[i],{focus:false,callback:false})}
        if(c[f]&&c[f]["onChange"]){
